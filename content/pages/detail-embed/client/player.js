...
$(document).ready( function() {
    ...
    window.onpopstate = function(event) {
		updateContents(window.location.search);
	}
...
function updateContents(resultURL) {
	...
	updateAjax = $.ajax({
		method: "POST",
		url: "./modules/player/content.player.php"+ resultURL + "&a=media"
	}).done(function(data) {
		$('#content').html($(data));
		updatePlayer();
        updateAutoplayState();
...
function updateQuery(resultID) {
    ...
    var locationString = 'play'+ location.search.replace(/(&playresults=[0-1])/, ''),
